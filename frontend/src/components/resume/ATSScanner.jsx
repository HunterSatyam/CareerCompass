        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-10 px-4 md:px-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden transform transition-all hover:shadow-2xl h-fit">
                        <div className="p-6 md:p-8 bg-gradient-to-br from-blue-50 to-white dark:from-gray-800 dark:to-gray-800">
                            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-6 flex items-center gap-2">
                                <Upload className="text-blue-600 dark:text-blue-500" /> Upload Your Details
                            </h2>
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="group">
                                    ...
                                    <div className="relative w-full">
                                        <label htmlFor="dropzone-file" className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl cursor-pointer transition-colors duration-300 ${file ? 'border-green-400 bg-green-50 dark:bg-green-900/20' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 hover:border-blue-400'}`}>
                                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                {file ? (
                                                    <>
                                                        ...
                                                        <p className="text-xs text-green-500 dark:text-green-300 mt-1">Ready to scan</p>
                                                    </>
                                                ) : (
                                                    <>
                                                        <Upload className="w-10 h-10 text-gray-400 mb-3 group-hover:text-blue-500 transition-colors" />
                                                        <p className="text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold text-blue-600 dark:text-blue-400">Click to upload</span> or drag and drop</p>
                                        ...
                                        {file && (
                                            <button
                                                ...
                                                className="absolute top-2 right-2 p-1 bg-white dark:bg-gray-800 rounded-full text-gray-400 hover:text-red-500 shadow-sm"
                                            >
                                                <XCircle size={20} />
                                            </button>
                                        )}
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="w-full p-4 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent min-h-[200px] text-sm leading-relaxed transition-shadow shadow-sm focus:shadow-md resize-none placeholder-gray-400 dark:placeholder-gray-500"
                    ...
                    <div className="space-y-6">
                        {!result ? (
                            <div className="h-full flex flex-col items-center justify-center p-12 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 text-center">
                                <div className="w-24 h-24 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
                                    <TrendingUp className="w-10 h-10 text-blue-400 dark:text-blue-500 opacity-50" />
                                </div>
                                <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">No Scan Results Yet</h3>
                            ...
                            <div className="space-y-6 animate-in slide-in-from-bottom-5 duration-700">
                                ...
                                <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 relative overflow-hidden">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                                        <div className="text-center md:text-left">
                                            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">ATS Compatibility Score</h3>
                                            <div className="flex items-baseline gap-2">
                                                <span className={`text-6xl font-black ${result.score >= 70 ? 'text-green-600' : result.score >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                                                    {result.score}
                                                </span>
                                                <span className="text-2xl text-gray-400 dark:text-gray-500 font-bold">/100</span>
                                            </div>
                                ...
                                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
                                    <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-2">
                                        <CheckCircle className="w-5 h-5 text-blue-500" /> Keyword Analysis
                                    </h3>

                                    <div className="space-y-4">
                                        <div>
                                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-2 font-medium">✅ Matched Keywords</p>
                                            <div className="flex flex-wrap gap-2">
                                                {result.matchedKeywords?.length > 0 ? (
                                                    result.matchedKeywords.map((k, i) => (
                                                        <span key={i} className="px-3 py-1 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded-full text-xs font-semibold lowercase">
                                                            {k}
                                                        </span>
                                                    ))
                                                ) : <span className="text-gray-400 dark:text-gray-500 italic text-sm">No exact matches found.</span>}
                                            </div>
                                        </div>

                                        <div>
                                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-2 font-medium">❌ Missing Keywords (Add these!)</p>
                                            <div className="flex flex-wrap gap-2">
                                                {result.missingKeywords?.length > 0 ? (
                                                    result.missingKeywords.slice(0, 15).map((k, i) => (
                                                        <span key={i} className="px-3 py-1 bg-red-50 dark:bg-red-900/40 text-red-600 dark:text-red-400 rounded-full text-xs font-semibold border border-red-100 dark:border-red-900/50 lowercase">
                                                            {k}
                                                        </span>
                                                    ))
                                ...
                                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <ul className="space-y-2">
                                                {criticalSuggestions.map((s, i) => (
                                                    <li key={i} className="p-3 bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-300 text-sm rounded-lg border border-red-100 dark:border-red-900/50 flex items-start gap-2">
                                        ...
                                        <div>
                                            ...
                                            <ul className="space-y-2">
                                                {recommendedSuggestions.map((s, i) => (
                                                    <li key={i} className="p-3 bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 text-sm rounded-lg border border-blue-100 dark:border-blue-900/50 flex items-start gap-2">
                                    ...
                                    {criticalSuggestions.length === 0 && recommendedSuggestions.length === 0 && (
                                        <div className="text-center py-6 text-green-600 dark:text-green-500 font-medium">
