...
const ResumePreview = ({ data, template = 'modern' }) => {
    ...
    if (template === 'classic') {
        return (
            <div className="w-full h-full bg-white shadow-2xl p-8 md:p-12 min-h-[1123px] text-gray-900 font-serif" id="resume-preview">
                {/* Header */}
                <header className="text-center border-b-[3px] border-double border-gray-800 pb-4 mb-6">
                    <h1 className="text-4xl font-bold uppercase tracking-widest mb-1">
                        {personalInfo.firstName} {personalInfo.lastName}
                    </h1>
                    <p className="text-lg italic text-gray-700 mb-3">{data.title}</p>
                                ...
                                {experience.map((exp, index) => (
                                    <div key={index}>
                                        <div className="flex justify-between font-bold text-gray-900">
                                            <span>{exp.company} {exp.location && <span className="font-normal italic">, {exp.location}</span>}</span>
                                            <span className="font-normal">{formatDate(exp.startDate)} - {exp.current ? 'Present' : formatDate(exp.endDate)}</span>
                                        </div>
                                        <div className="italic text-sm mb-1">{exp.role}</div>
                                        <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-line pl-4 border-l-2 border-gray-200">
                                            {exp.description}
                                        </p>
                                    </div>
                                ))}
                                ...
                                {education.map((edu, index) => (
                                    <div key={index} className="flex justify-between items-baseline">
                                        <div>
                                            ...
                                            <span className="italic text-sm ml-2">- {edu.degree}</span>
                                        </div>
                                        <span className="text-sm">{formatDate(edu.startDate)} - {edu.current ? 'Present' : formatDate(edu.endDate)}</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                    ...
                    {projects?.length > 0 && (
                        <section>
                            ...
                            <div className="space-y-3">
                                {projects.map((proj, index) => (
                                    <div key={index}>
                                        <div className="font-bold text-gray-900">{proj.title} <span className="font-normal text-sm italic border-l border-gray-300 pl-2 ml-2">{proj.technologies}</span></div>
    ...
    if (template === 'minimalist') {
        return (
            <div className="w-full h-full bg-white shadow-2xl p-10 min-h-[1123px] text-gray-800 font-sans tracking-wide" id="resume-preview">
                {/* Header */}
                <header className="mb-8">
                    <h1 className="text-3xl font-light tracking-tight mb-2 text-black">
                ...
                <div className="space-y-8">
                    ...
                    {personalInfo.summary && (
                        <section className="grid grid-cols-4 gap-4">
                            <h2 className="col-span-1 text-xs font-bold uppercase tracking-widest text-gray-400 mt-1">Profile</h2>
                            <p className="col-span-3 text-sm leading-relaxed text-gray-700 whitespace-pre-line">
                                {personalInfo.summary}
                            </p>
                        </section>
                    )}
                    ...
                    {experience?.length > 0 && (
                        <section className="grid grid-cols-4 gap-4">
                            <h2 className="col-span-1 text-xs font-bold uppercase tracking-widest text-gray-400 mt-1">Experience</h2>
                    ...
                    {education?.length > 0 && (
                        <section className="grid grid-cols-4 gap-4">
                            <h2 className="col-span-1 text-xs font-bold uppercase tracking-widest text-gray-400 mt-1">Education</h2>
                            <div className="col-span-3 space-y-4">
                                {education.map((edu, index) => (
                                    <div key={index}>
                                        <div className="flex justify-between items-baseline">
                                            <h3 className="font-bold text-black">{edu.degree}</h3>
                                            <span className="text-xs text-gray-500">{formatDate(edu.startDate)} - {edu.current ? 'Present' : formatDate(edu.endDate)}</span>
                                        </div>
                    ...
                    {projects?.length > 0 && (
                        <section className="grid grid-cols-4 gap-4">
                            <h2 className="col-span-1 text-xs font-bold uppercase tracking-widest text-gray-400 mt-1">Projects</h2>
                            <div className="col-span-3 space-y-4">
                                {projects.map((proj, index) => (
                                    <div key={index}>
                                        <h3 className="font-bold text-black">{proj.title}</h3>
                                        <p className="text-xs text-gray-500 mt-0.5 mb-1">{proj.technologies}</p>
                    ...
                    {skills && (
                        <section className="grid grid-cols-4 gap-4">
                            <h2 className="col-span-1 text-xs font-bold uppercase tracking-widest text-gray-400 mt-1">Skills</h2>
                            <div className="col-span-3 text-sm text-gray-700 leading-relaxed">
                                {skills}
                            </div>
                        </section>
                    )}
