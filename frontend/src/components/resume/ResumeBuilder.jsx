...
const ResumeBuilder = () => {
    // Style Constants
    const labelClass = "block text-xs font-bold text-gray-800 dark:text-gray-200 uppercase tracking-wider mb-2.5 ml-1";
    const inputClass = "w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm text-gray-900 dark:text-white placeholder:text-gray-400 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-md";
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-950 overflow-hidden">
            {/* Top Navigation Bar */}
            <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-6 z-20 shrink-0">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg">
                        <Layout size={20} />
                    </div>
                    <h1 className="font-bold text-gray-800 dark:text-white text-lg">Resume<span className="text-blue-600 dark:text-blue-500">Builder</span></h1>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <select 
                        ...
                        className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm font-bold rounded-lg px-3 py-2 border border-gray-300 dark:border-gray-700 outline-none"
                    >
                        <option value="modern">Modern</option>
                        <option value="classic">Classic</option>
                        <option value="minimalist">Minimalist</option>
                    </select>
                    <button className="md:hidden p-2 text-gray-600 dark:text-gray-400" onClick={() => setActiveTab(activeTab === 'edit' ? 'preview' : 'edit')}>
                        {activeTab === 'edit' ? <Eye /> : <Layout />}
                    </button>
                    <button onClick={handleSubmit} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white text-sm font-medium rounded-lg hover:bg-black dark:hover:bg-gray-600 transition-colors">
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* Left Sidebar - Steps */}
                <aside className="w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-col hidden md:flex shrink-0">
                    <div className="p-6">
                        <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-4">Sections</p>
                            ...
                            {steps.map((step) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${currentStep === step.id
                                        ? "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 shadow-sm ring-1 ring-blue-100 dark:ring-blue-900/50"
                ...
                <main className="flex-1 flex overflow-hidden relative">
                    {/* Form Panel */}
                    <div className={`flex-1 overflow-y-auto p-6 md:p-10 transition-transform duration-300 ${activeTab === 'preview' ? '-translate-x-full absolute w-full' : 'translate-x-0'} md:translate-x-0 md:static md:w-1/2 bg-gray-50/50 dark:bg-gray-900/50`}>
                        <div className="max-w-2xl mx-auto pb-10">
                            <div className="mb-8">
                                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{steps[currentStep - 1].title}</h2>
                                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Fill in the details below to complete your resume.</p>
                            </div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                                            <div className="grid grid-cols-2 gap-5">
                                                ...
                                                <div className="col-span-2">
                                                    ...
                                                    <textarea name="summary" value={resumeData.personalInfo.summary} onChange={(e) => handleInputChange(e, 'personalInfo')} className={`${inputClass} h-32 resize-none leading-relaxed`} placeholder="Write a brief summary of your career, key skills, and what you bring to the table..." />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {currentStep === 2 && (
                                    <div className="space-y-6">
                                        {resumeData.experience.map((exp, index) => (
                                            <div key={index} className="p-5 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 relative group transition-all hover:shadow-md">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="date" value={exp.endDate} onChange={(e) => handleInputChange(e, 'experience', index, 'endDate')} disabled={exp.current} className={`${inputClass} disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:text-gray-400`} />
                                                    </div>
                                                    <div className="col-span-2">
                                                        <label className={labelClass}>Description</label>
                                                        <textarea value={exp.description} onChange={(e) => handleInputChange(e, 'experience', index, 'description')} className={`${inputClass} h-32 resize-none`} placeholder="Describe your responsibilities and achievements..." />
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        <button onClick={() => addField('experience', { company: "", role: "", startDate: "", endDate: "", current: false, description: "", location: "" })} className="w-full py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-500 dark:text-gray-400 font-medium hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all flex items-center justify-center gap-2">
                                        ...
                                        {resumeData.projects.map((proj, index) => (
                                            <div key={index} className="p-5 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 relative group transition-all hover:shadow-md">
                                                ...
                                                <div className="grid grid-cols-1 gap-4">
                                                    ...
                                                    <div>
                                                        <label className={labelClass}>Description</label>
                                                        <textarea value={proj.description} onChange={(e) => handleInputChange(e, 'projects', index, 'description')} className={`${inputClass} h-24 resize-none`} placeholder="What did you build and what impact did it have?" />
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        <button onClick={() => addField('projects', { title: "", description: "", technologies: "", link: "" })} className="w-full py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-500 dark:text-gray-400 font-medium hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all flex items-center justify-center gap-2">
                                        ...
                                        {resumeData.education.map((edu, index) => (
                                            <div key={index} className="p-5 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 relative group transition-all hover:shadow-md">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div>
                                                        <label className={labelClass}>End Date</label>
                                                        <input type="date" value={edu.endDate} onChange={(e) => handleInputChange(e, 'education', index, 'endDate')} disabled={edu.current} className={`${inputClass} disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:text-gray-400`} />
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        <button onClick={() => addField('education', { institution: "", degree: "", startDate: "", endDate: "", current: false })} className="w-full py-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-500 dark:text-gray-400 font-medium hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all flex items-center justify-center gap-2">
                                ...
                                {currentStep === 5 && (
                                    <div className="space-y-4">
                                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                            <label className="label mb-2 block text-lg font-bold text-gray-800 dark:text-gray-200">Skills</label>
                                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Add your technical skills separated by commas.</p>
                                            <textarea value={resumeData.skills} onChange={(e) => handleInputChange(e, 'skills')} className={`${inputClass} h-40 text-base leading-relaxed`} placeholder="e.g. React, Node.js, Python, Project Management, SEO..." />
                                        </div>
                                    </div>
                                )}
                            </div>
                            {/* Navigation Buttons - Static at bottom of content */}
                            <div className="mt-8 flex justify-between items-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-xl shadow-sm">
                                <button onClick={prevStep} disabled={currentStep === 1} className="flex items-center gap-2 px-6 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-medium rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 transition-colors">
                                    <ArrowLeft size={18} /> Back
                                </button>
                                <div className="flex gap-2">
                                    {currentStep < steps.length && (
                                        <button onClick={nextStep} className="flex items-center gap-2 px-6 py-2.5 bg-gray-900 dark:bg-gray-700 text-white font-medium rounded-full hover:bg-black dark:hover:bg-gray-600 transition-colors shadow-lg shadow-gray-200 dark:shadow-none">
                                            Next Step <ArrowRight size={18} />
                                        </button>
                                    )}
                                    {currentStep === steps.length && (
                                        <button onClick={handleSubmit} disabled={loading} className="flex items-center gap-2 px-6 py-2.5 bg-green-600 text-white font-medium rounded-full hover:bg-green-700 transition-colors shadow-lg shadow-green-200 dark:shadow-none">
                    ...
                    <div className={`flex-1 bg-gray-200 dark:bg-gray-800 overflow-y-auto p-4 md:p-8 transition-transform duration-300 ${activeTab === 'edit' ? 'translate-x-full absolute w-full' : 'translate-x-0'} md:translate-x-0 md:static md:block hidden-scrollbar`}>
