...
const POST_TYPES = [
    ...
    { id: 'internship', title: 'Internship', icon: <Clock />, color: 'emerald', desc: 'Find bright applicants for your team' },
                ...
                {(selectedType === 'hackathon' || selectedType === 'competition') && (
                    <>
                        <div>
                            ...
                            <div className='relative'>
                                <Trophy className='absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 dark:text-zinc-600' size={18} />
                        ...
                        <div>
                            ...
                            <input type="date" name="registrationDeadline" value={formData.registrationDeadline} onChange={handleInputChange} className={`${inputBase} [color-scheme:light] dark:[color-scheme:dark]`} />
                        </div>
                        <div className='md:col-span-2'>
                ...
                {(selectedType === 'webinar' || selectedType === 'hackathon' || selectedType === 'competition') && (
                    <>
                        <div>
                            ...
                            <input type="date" name="date" value={formData.date} onChange={handleInputChange} className={`${inputBase} [color-scheme:light] dark:[color-scheme:dark]`} />
                        </div>
                        <div>
                            <label className={labelBase}>Start Time</label>
                            <input type="time" name="time" value={formData.time} onChange={handleInputChange} className={`${inputBase} [color-scheme:light] dark:[color-scheme:dark]`} />
                        </div>
                    </>
                )}
            ...
            <div className='max-w-5xl mx-auto px-4 mt-16'>
                ...
                <div className='text-center mb-16'>
                    <motion.div
                        ...
                        className='inline-flex items-center gap-2 bg-purple-50 dark:bg-purple-900/20 px-4 py-2 rounded-full text-purple-600 dark:text-purple-400 font-black text-[10px] uppercase tracking-widest mb-6 border border-purple-100 dark:border-purple-900/30 shadow-sm'
                    >
                        <Sparkles size={14} /> Recruiter Studio
                    </motion.div>
                    <h1 className='text-5xl md:text-6xl font-black text-gray-900 dark:text-white tracking-tight mb-4 uppercase'>Publish <span className='text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 dark:from-purple-400 dark:to-indigo-400'>Excellence</span></h1>
                    <p className='text-gray-500 dark:text-zinc-500 font-medium text-lg'>Reach thousands of motivated applicants and professionals instantly.</p>
                </div>

                {/* Progress Bar */}
                <div className='mb-20 relative h-2 w-full bg-gray-100 dark:bg-zinc-800 rounded-full overflow-hidden shadow-inner'>
                    <motion.div
                        initial={{ width: '0%' }}
                        animate={{ width: step === 1 ? '33%' : '100%' }}
                        transition={{ type: "spring", stiffness: 50, damping: 20 }}
                        className='absolute top-0 left-0 h-full bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 shadow-[0_0_15px_rgba(79,70,229,0.5)]'
                    ></motion.div>
                </div>
                ...
                <AnimatePresence mode='wait'>
                    {step === 1 ? (
                        ...
                        >
                            {POST_TYPES.map((type) => (
                                <motion.div
                                    ...
                                    className={`group cursor-pointer p-8 rounded-[40px] border-2 transition-all relative overflow-hidden ${selectedType === type.id
                                        ? 'border-purple-600 dark:border-purple-500 bg-white dark:bg-zinc-900 shadow-2xl'
                                        : 'border-white dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-purple-100 dark:hover:border-purple-900/50 shadow-xl dark:shadow-none'
                                        }`}
                                >
                                    <div className={`w-20 h-20 rounded-[24px] flex items-center justify-center mb-8 text-white text-3xl shadow-xl transition-all duration-500 group-hover:rotate-6 ${type.color === 'blue' ? 'bg-blue-600' :
                                        type.color === 'emerald' ? 'bg-emerald-600' :
                                            type.color === 'amber' ? 'bg-amber-600' :
                                    ...
                                    <h3 className='text-2xl font-black text-gray-900 dark:text-white mb-2 uppercase tracking-tight'>{type.title}</h3>
                                    <p className='text-gray-500 dark:text-zinc-500 font-bold text-xs uppercase tracking-wider leading-relaxed'>{type.desc}</p>

                                    <div className='mt-10 flex items-center gap-2 text-purple-600 dark:text-purple-400 font-black text-[10px] uppercase tracking-[0.2em] transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all'>
                        ...
                        <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: -40 }}
                            className='bg-white dark:bg-zinc-900 rounded-[48px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.08)] dark:shadow-none border border-white/50 dark:border-zinc-800 p-10 md:p-16 relative overflow-hidden'
                        >
                            ...
                            >
                                <div className='p-3 bg-gray-50 dark:bg-zinc-800 rounded-2xl group-hover:bg-purple-50 dark:group-hover:bg-purple-900/20 transition-all'>
                                    <ChevronLeft size={20} />
                                </div>
                                Back to Selection
                            </button>
                            ...
                            <div className='flex items-center gap-6 mb-16 pb-12 border-b border-gray-100 dark:border-zinc-800'>
                                <div className='w-20 h-20 bg-black dark:bg-white text-white dark:text-black rounded-[28px] flex items-center justify-center text-3xl shadow-2xl relative'>
                                    ...
                                    <div className='absolute -bottom-2 -right-2 w-8 h-8 bg-emerald-500 rounded-full border-4 border-white dark:border-zinc-900 flex items-center justify-center'>
                                        <CheckCircle2 size={14} className='text-white' />
                                    </div>
                                </div>
                                <div>
                                    <h2 className='text-4xl font-black text-gray-900 dark:text-white uppercase tracking-tight'>Deploying {POST_TYPES.find(t => t.id === selectedType)?.title}</h2>
                                    <p className='text-gray-400 dark:text-zinc-500 font-black text-xs uppercase tracking-[0.2em] mt-2'>Step 02: Intelligence Configuration</p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className='space-y-16'>
                                ...
                                <div className='pt-12 flex flex-col md:flex-row gap-6'>
                                    <Button
                                        type="submit"
                                        disabled={loading}
                                        className='flex-1 h-24 text-xl font-black uppercase tracking-[0.2em] rounded-[32px] bg-black dark:bg-white text-white dark:text-black hover:bg-gray-900 dark:hover:bg-gray-100 shadow-2xl transition-all active:scale-95 disabled:bg-gray-200 dark:disabled:bg-zinc-800 disabled:text-gray-400 border-none group overflow-hidden relative'
                                    >
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        onClick={() => navigate('/')}
                                        className='md:w-[240px] h-24 rounded-[32px] border-2 border-gray-100 dark:border-zinc-800 hover:bg-rose-50 dark:hover:bg-rose-900/10 hover:border-rose-100 dark:hover:border-rose-900/30 hover:text-rose-600 dark:hover:text-rose-400 font-black uppercase tracking-[0.2em] text-gray-400 text-sm transition-all bg-white dark:bg-zinc-900'
                                    >
                                        Abort Mission
                                    </Button>
                                </div>
